...
namespace Rainbow.Tfs.SourceControl
{
	public class TfsFileHandler
	{
		...
		private void AssertFileExistsOnFileSystem()
		{
			...
			{
				throw new Exception("[Rainbow.Tfs] TFS File Handler: file does not exist on disk for " + _filename);
			}
		}
		...
		private void AssertFileExistsInTfs()
		{
			...
			{
				throw new Exception("[Rainbow.Tfs] TFS File Handler: file does not exist in TFS for " + _filename);
			}
		}
		...
		private void AssertFileDoesNotExistInTfs()
		{
			if (FileExistsOnServer)
			{
				throw new Exception("[Rainbow.Tfs] TFS File Handler: file exists in TFS for " + _filename);
			}
		}
		...
		public virtual bool GetFileExistsOnServer()
		{
			...
			{
				Sitecore.Diagnostics.Log.Error("[Rainbow.Tfs] TFS File Handler: Could not communicate with TFS Server for " + _filename, ex, this);
				throw;
			}
		...
		protected virtual void UndoNonMatchingPendingChanges(ChangeType changeType)
		{
			...
			{
				Sitecore.Diagnostics.Log.Error("[Rainbow.Tfs] TFS File Handler: Could not revert pending change for " + _filename, ex, this);
				throw;
			}
		}
		...
		private void TryRefreshLocalWithTfs()
		{
			...
			{
				Sitecore.Diagnostics.Log.Error("[Rainbow.Tfs] TFS File Handler: Could not refresh local from TFS " + _filename, ex, this);
				throw;
			}
		}
		...
		public bool CheckoutFileForDelete()
		{
			...
			if (!FileExistsOnFileSystem)
			{
				Sitecore.Diagnostics.Log.Warn("[Rainbow.Tfs] TFS File Handler: Attempting to delete a file that doesn't exist on the local filesystem for " + _filename, this);
			}
		...
		protected virtual int AddFileToTfs()
		{
			...
			{
				Sitecore.Diagnostics.Log.Error("[Rainbow.Tfs] TFS File Handler: Could not add file to TFS for " + _filename, ex, this);
				throw;
			}
		}
		...
		protected virtual int EditFileInTfs()
		{
			...
			{
				Sitecore.Diagnostics.Log.Error("[Rainbow.Tfs] TFS File Handler: Could not checkout file in TFS for " + _filename, ex, this);
				throw;
			}
		}
		...
		private void OnNonFatalError(Object sender, ExceptionEventArgs e)
		{
			...
			Sitecore.Diagnostics.Log.Error("[Rainbow.Tfs] TFS File Handler: Non-fatal exception: " + message, this);
		}
	}
}